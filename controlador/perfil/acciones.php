...
try
{
	...
	if($accion==1)
	{
		...
		$resultados=$perfil->Buscar();
		$jsondata['success'] = true;
		$jsondata['code'] = '00005';
		$jsondata['description'] = "select perfil llenado correctmente";
		$jsondata['data']="<option value=''> </option>";
		while($row=$perfil->Siguiente($resultados))
		{
            $jsondata['data']=$jsondata['data']."<option value='".$row['IdPerfil']."'>".$row['NombrePerfil']."</option>";
		}
	}
...
{
	$log->write('Error en el controlado de acciones perfil '.$e->getMessage());
}
?>